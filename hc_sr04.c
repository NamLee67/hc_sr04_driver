...
#define CLASS_NAME "hc_sr04_class"
...
int hc_sr04_open(struct inode *inode, struct file *filp) {
    ...
    if (atomic_inc_and_test(&opened)) {        // atomic_inc sẽ kiểm tra xem biến opened có lên 1 hay không 
        return 0;                              // test xem giá trị đã tăng lên 1 ch
    }
    else {
        return -EBUSY;
    }
...
static int hc_sr04_init(void)
{
    ...
    result = alloc_chrdev_region(&dev, 0, 1, "hc_sr04");
    if (result < 0) {
    ...
    if (gpio_is_valid(GPIO_TRIGGER) == false)   // kiểm tra xem giá trị của chân trigger có tồn tại k, hợp lệ k
    {
        pr_err("[HC-SR04]: GPIO %d is not valid\n", GPIO_TRIGGER);
        return -1;
    }

    if (gpio_request(GPIO_TRIGGER, "GPIO_TRIGGER") < 0)     // kiểm tra quyền điều khiển cho chân trigger
    {
    ...
    gpio_direction_output(GPIO_TRIGGER, 0);  // thiết lập out put cho chan trigger, 0: gia tri cua chan ban dau
    gpio_set_value(GPIO_TRIGGER, 0);        // khoi tao gia tri cho chan trigger
    ...
    gpio_direction_input(GPIO_ECHO);    // thiet lap input cho chan echo

    gpio_irq_number = gpio_to_irq(GPIO_ECHO);   // chuyen gpio sang ngat cho chan echo
  
    if (request_irq(gpio_irq_number,           // yeu cau ngat
